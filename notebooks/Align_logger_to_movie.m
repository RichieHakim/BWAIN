    ...
    if size(dirProps,1) > 0
        ...
        if size(fileNames,1) > lastImportedFileIdx
            if fopen([directory_movie, '\', fileNames(lastImportedFileIdx+1,:)]) ~= -1
                ...
                movie_chunk_hash{iter_file} = compute_hashSequence(movie_chunk, 1);
...
%%
movie_hash = cat(1, movie_chunk_hash{:});

%%
hash1 = logger.timeSeries(:,27);
hash2 = movie_hash;
...
logger_aligned.trials = logger.trials;
logger_aligned.trials(:,3) = indexWithNaNs(ia_2to1, logger.trials(:,3));
logger_aligned.trials(:,9) = indexWithNaNs(ia_2to1, logger.trials(:,9));

logger_valsROIs_aligned = indexWithNaNs(logger_valsROIs , ia_2to1);
%%
figure; plot(hash2(:,1))
hold on; plot(logger_aligned.timeSeries(:,27))
%% Save aligned logger
