...
%% Trial prep
    ...
    CS_quiescence = algorithm_quiescence(data.ROIs.cursor_brain, threshold_quiescence);
    CS_threshold = algorithm_thresholdState(data.ROIs.cursor_output, params.cursor.threshold_value);
...
%     end
    ...
    if sm.CE_waitForBaseline && (sm.counter_quiescenceHold > duration_quiescenceHold*frameRate)
...
%         source.hSI.task_cursorGoalPos.writeAnalogData(3.2);
    ...
    % END TRIAL: FAILURE
    if sm.CE_trial && sm.counter_trialIdx >= round(frameRate * duration_trial)
        sm.CE_trial = 0;
        sm.ET_timeout = 1;
...
%         source.hSI.task_goalAmplitude.writeDigitalData(0);
    ...
    % END TIMEOUT
    if sm.CE_timeout && sm.counter_timeout >= round(frameRate * duration_timeout)
        sm.CE_timeout = 0;
        sm.ET_ITI_withZ = 1;
    end
    
    % END TRIAL: THRESHOLD REACHED
    if sm.CE_trial && sm.counter_CS_threshold >= round(frameRate * duration_threshold)
...
%         %         giveReward3(source, 1, 0, 500, reward_delay, LED_duration, 1, LED_ramp_duration); % in ms. This is in the START section so that it only delivers once
    ...
    % END DELIVER REWARD
    if sm.CE_rewardDelivery && sm.counter_rewardDelivery >= round(frameRate * duration_rewardDelivery)
...
%         source.hSI.task_goalAmplitude.writeDigitalData(0);
        ...
        if frameNum > (sm.counter_last_z_correction + interval_z_correction)
...
%                 currentPosition = moveFastZ(source, [], clampedDelta, [], [20,380]);
    ...
    if sm.CE_ITI_withZ && sm.counter_ITI_withZ >= round(frameRate * duration_ITI)
...
if sm.CE_experimentRunning
    if sm.frequencyOverride
        ...
        voltage_cursorCurrentPos = convert_cursor_to_voltage(data.ROIs.cursor_output , params.cursor.bounds_cursor, params.cursor.voltage_at_threshold);
    end
...
%
%     plotUpdatedOutput2([CE_waitForBaseline*.9, sm.CE_trial*.8, CE_rewardToneHold*.7,...
%         CE_rewardDelivery*.6, CE_ITI_successful*.5, CE_timeout*.4,...
%         CE_buildingUpStats*.3, CE_experimentRunning*.2]...
%         , duration_plotting, frameRate, 'Rewards', 10, 52, ['Num of Rewards:  ' , num2str(sm.NumOfRewardsAcquired)])
%
...
%     plotUpdatedOutput6(data.ROIs.cursor_output , duration_plotting , frameRate , 'cursor' , 10,5)
%     drawnow
...
if ~isnan(frameNum)
    ...
    logger.timeSeries(frameNum,29) = sm.fakeFeedback_inUse;
    logger.timeSeries(frameNum,30) = sm.trialType_cursorOn;
    logger.timeSeries(frameNum,31) = sm.trialType_feedbackLinked;
    logger.timeSeries(frameNum,32) = sm.trialType_rewardOn;
    logger.timeSeries(frameNum,33) = sm.counter_last_z_correction;
...
if frameNum == last_frameNum
    ...
    NumOfRewardsAcquired = sm.NumOfRewardsAcquired;
else
    logger_output = [];
    loggerNames_output = [];
...
%% FUNCTIONS
    ...
    function startSession
        ...
        loggerNames.timeSeries{33} = 'counter_last_z_correction';
...
%         sm.CE_rewardDelivery = 0;
%         sm.counter_rewardDelivery = 0;
%         sm.ET_ITI_successful = 0;
%         sm.CE_ITI_successful = 0;
%         sm.counter_ITI_successful = 0;
%         sm.ET_waitForBaseline = 0;
%         sm.CE_waitForBaseline = 0;
%         sm.ET_timeout = 0;
%         sm.CE_timeout = 0;
