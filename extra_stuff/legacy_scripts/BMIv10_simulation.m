function [logger_output , logger_valsROIs_output , NumOfRewardsAcquired] = BMIv10_simulation(vals_neurons, counter_frameNum , cellWeightings , last_frameNum , threshold_reward , threshold_quiescence)
%% Variable stuff
% tic
...
numCells = length(cellWeightings);
...
%     if counter_frameNum < duration_rollingStats
%         movstdWindow_moving(vals_neurons);
%         movmeanWindow_moving(vals_neurons);
%         F_std = movstdWindow_running(vals_neurons);
%         F_mean = movmeanWindow_running(vals_neurons);
%     else
%         F_std = movstdWindow_moving(vals_neurons);
%         F_mean = movmeanWindow_moving(vals_neurons);
%     end
...
%     cursor = F_zscore * baselineStuff.ROIs.cellWeightings;
    cursor_raw = F_zscore * cellWeightings;
    logger.decoder_outputs(counter_frameNum,2) = cursor_raw;
    if counter_frameNum >= win_smooth+2
        cursor = nanmean(logger.decoder_outputs(counter_frameNum-(win_smooth-1):counter_frameNum,2));
    else
        cursor = cursor_raw;
    end
    
    CS_quiescence = algorithm_quiescence(cursor, threshold_quiescence);
    CS_threshold = algorithm_thresholdState(cursor, threshold_reward);
%     toc
    %%  ===== TRIAL STRUCTURE =====
    % CE = current epoch
...
if CE_experimentRunning
    if frequencyOverride
        freqToOutput = cursorToFrequency(threshold_reward, range_cursorSound, range_freqOutput);
    else
...
if ~isnan(counter_frameNum)
    ...
    logger.decoder_outputs(counter_frameNum,1) = cursor;

    logger.decoder_outputs(counter_frameNum,3) = freqToOutput;
    logger.decoder_outputs(counter_frameNum,4) = outputVoltageToTeensy;
    %     logger.decoder_outputs.rawVals(counter_frameNum,:) = vals;
    %     logger.decoder_outputs_outputs.dFoF(counter_frameNum,:) = dFoF;
...
%% FUNCTIONS
    function startSession
        ...
        logger.decoder_outputs_outputs = NaN(duration_session,4);
        %         logger.decoder_outputs_outputs_outputs.dFoF = NaN(duration_session,1);
