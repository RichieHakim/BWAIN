function outputVoltageToTeensy = BMI_trial_MOTION_CORRECTION_TEENSY(currentImage,varargin)
% function freqToOutput = BMI_trial_MOTION_CORRECTION_TEENSY(currentImage,varargin)
%% Variable stuff
global counter_frameNum counter_CS_threshold counter_trialDuration counter_timeout counter_rewardToneHold...
    ...
    vals img_MC_moving img_ROI_corrected...
    
% clear baselineStuff2 BMIstuff2
persistent baselineStuff2 BMIstuff2

%    baselineStuff = load('baselineStuff');
%     BMIstuff = load('BMIstuff');
%     
if ~isstruct(baselineStuff2)
   baselineStuff = load('baselineStuff');
    BMIstuff = load('BMIstuff');
    
    baselineStuff2 = baselineStuff.baselineStuff;
    BMIstuff2 = BMIstuff.BMIstuff;
% 1
% baselineStuff
end
% class baselineStuff2
% baselineStuff2
% length(fieldnames(baselineStuff2))
...
% scale_factors = baselineStuff2.scale_factors;
...
% directory = [baselineStuff2.directory, '\expParams.mat'];
...
%% == MOTION CORRECT ROIs ==
if numel(baselineStuff2.idxBounds_ROI) ~= numCells
    error('NUMBER OF ROIs IS DIFFERENT THAN NUMBER OF DESIRED CELLS.  RH20191215')
end
% if numel(img_ROI_moving) ~= numCells
%     img_ROI_moving = [];
% end
...
for ii = 1:numCells
    img_MC_moving{ii} = currentImage(baselineStuff2.idxBounds_imgMC{ii}(1,2):baselineStuff2.idxBounds_imgMC{ii}(2,2)  ,...
        baselineStuff2.idxBounds_imgMC{ii}(1,1):baselineStuff2.idxBounds_imgMC{ii}(2,1)); % note that idxBounds_ROI will be [[x1;x2] , [y1;y2]]
    [xShift(ii) , yShift(ii) , cxx{ii} , cyy{ii}] = motionCorrection_ROI(img_MC_moving{ii} , BMIstuff2.img_MC_reference{ii});
         
%     (baselineStuff2.idxBounds_ROI{ii}(1,1):baselineStuff2.idxBounds_ROI{ii}(2,1)) +round(xShift(ii))
    
%     img_ROI_corrected{ii} = currentImage(baselineStuff2.idxBounds_ROI{ii}(1,2):baselineStuff2.idxBounds_ROI{ii}(2,2)  ,  baselineStuff2.idxBounds_ROI{ii}(1,1):baselineStuff2.idxBounds_ROI{ii}(2,1)); % note that idxBounds_ROI will be [[x1;x2] , [y1;y2]]
    img_ROI_corrected{ii} = currentImage((baselineStuff2.idxBounds_ROI{ii}(1,2):baselineStuff2.idxBounds_ROI{ii}(2,2)) +round(yShift(ii)) ,...
        (baselineStuff2.idxBounds_ROI{ii}(1,1):baselineStuff2.idxBounds_ROI{ii}(2,1)) +round(xShift(ii))); % note that idxBounds_ROI will be [[x1;x2] , [y1;y2]]
end
% x = xShift(1)
% y = yShift(1)
% figure; plot(cyy{1})
% figure; imagesc(BMIstuff2.img_MC_reference{1})
...
%         plotUpdatedOutput([cursor , dFoF], duration_plotting, frameRate, 'Cursor , E1 , E2', 10, 10)
% 
...
%     
% if mod(counter_frameNum,10) == 0
%         plotUpdatedMotionCorrectionImages(BMIstuff2.img_MC_reference, img_MC_moving , img_ROI_corrected, 'Motion Correction')
% end
end
...
%         loggerTable = table;
%         loggerTable.Variables = logger;
%         loggerTable = mergevars(loggerTable, [29:29+numCells-1]);
%         loggerTable = mergevars(loggerTable, [29:29+numCells-1]+1);
        
        save([directory , '\logger.mat'], 'logger')
%         save([directory , '\loggerTable.mat'], 'loggerTable')
        saveParams(directory)
        disp('SESSION OVER')
...
%         CE_experimentRunning = 0;
    ...
    function saveParams(directory)
        ...
        save([directory , '\BMIstuff.mat'], 'BMIstuff2')
     end
end