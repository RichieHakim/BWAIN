% function outputVoltageToTeensy = userFunction_BMIv4(source, event, varargin)
function [ currentCursor , currentCSThresh, currentCSQuiescence, currentCETrial,  currentCEReward, currentCETimeout ]...
    = BMI_trial_simulation2(vals,startSession_pref, scale_factors, threshold_value, duration_session, duration_trial, ensemble_assignments, varargin)
%% Variable stuff
persistent counter_frameNum counter_CS_threshold counter_trialDuration counter_timeout counter_rewardToneHold...
...
% persistent baselineStuff motionCorrectionRefImages

% currentImage = source.hSI.hDisplay.lastFrame{1};

% % whos('motionCorrectionRefImages')
% if ~isstruct(baselineStuff)
%     load('baselineStuff');
% end
% if ~isstruct(motionCorrectionRefImages)
%     load('motionCorrectionRefImages');
% end
%     load('baselineStuff');
%     load('motionCorrectionRefImages');
%% == USER SETTINGS ==
...
% directory = 'F:\RH_Local\Rich data\scanimage data\mouse 12.5\01122020_test';
...
%     [xShift(ii) , yShift(ii), cxx{ii}, cyy{ii}] = motionCorrection_ROI(img_MC_moving_rollingAvg{ii} , motionCorrectionRefImages.img_MC_reference{ii});
...
%         %         plotUpdatedMotionCorrectionImages(motionCorrectionRefImages.img_MC_reference, img_MC_moving , img_ROI_corrected, 'Motion Correction')
%         plotUpdatedMotionCorrectionImages(motionCorrectionRefImages.img_MC_reference, img_MC_moving_rollingAvg , img_ROI_corrected, 'Motion Correction')
%     end
% end
...
currentCSThresh = CS_threshold;
currentCSQuiescence = CS_quiescence;
currentCETrial = CE_trial;
currentCEReward = CE_rewardDelivery;
currentCETimeout = CE_timeout;
%% End Session
