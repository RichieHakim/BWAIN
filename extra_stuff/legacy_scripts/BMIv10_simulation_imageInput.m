...
function [logger_output , logger_valsROIs_output , logger_valsROIs_output_zscored , NumOfRewardsAcquired] = BMIv10_simulation_imageInput(currentImage, counter_frameNum , baselineStuff , last_frameNum , threshold_value , threshold_quiescence)
%% Variable stuff
% tic
...
if ~isa(img_MC_moving_rolling, 'uint16')
    ...
    img_MC_moving_rolling = uint16(zeros(size(img_MC_moving,1) , size(img_MC_moving,2) , numFramesToAvgForMotionCorr));
end

% clear img_MC_moving_rolling
...
if CE_experimentRunning
    ...
    logger_valsROIs_zscored(counter_frameNum,:) = F_zscore;
...
if ~isnan(counter_frameNum)
    ...
    logger.decoder_outputs(counter_frameNum,2) = freqToOutput;
    logger.decoder_outputs(counter_frameNum,3) = outputVoltageToTeensy;
...
%% FUNCTIONS
    function startSession
        ...
        logger.decoder_outputs = NaN(duration_session,3);
        ...
        logger_valsROIs_zscored = nan(duration_session , numCells);
...
if counter_frameNum == last_frameNum
    ...
    logger_valsROIs_output_zscored = logger_valsROIs_zscored;
else
    logger_output = [];
    logger_valsROIs_output = [];
    logger_valsROIs_output_zscored = [];
end
% toc
end