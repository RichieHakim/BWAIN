function outputVoltageToTeensy = userFunction_BMIv7_manyCells(source, event, varargin)
%% Variable stuff
% tic
...
directory = 'D:\RH_local\data\scanimage data\round 4 experiments\mouse 6.28\20201216';
...
% class(currentImage)
% class(image_footprints_MC_weighted)
...
idx_safe = (x_idx < 1024) &...
            ...
            (y_idx < 512) & ...
            (y_idx > 0);
% num_unsafe = sum(~idx_safe);
% disp(num_unsafe)


image_footprints_MC_weighted(sub2ind([size(currentImage,1),size(currentImage,2)] , y_idx(idx_safe) , x_idx(idx_safe))) = ...
    baselineStuff.ROIs.spatial_footprints_tall_warped_weighted(idx_safe,4);

vals = sum(sum(bsxfun( @times , image_footprints_MC_weighted , single(currentImage) )));

% figure;
% imagesc(img_MC_moving_rollingAvg)
% figure;
% imagesc(baselineStuff.MC.meanImForMC_crop)
%% == ROLLING STATS ==
if CE_experimentRunning
%     logger.decoder.rawVals(1:counter_frameNum, :)
    ...
    if (CE_buildingUpStats == 1) && (counter_frameNum > 1)
        ...
        runningVals = [nan(duration_rollingStats, 1) ; logger.decoder.rawVals(1:counter_frameNum, :)];
            ...
            runningVals = [nan(duration_rollingStats, 1) ; vals'];
        end
    end
