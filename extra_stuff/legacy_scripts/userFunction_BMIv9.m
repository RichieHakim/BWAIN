function outputVoltageToTeensy = userFunction_BMIv8(source, event, varargin)
%% Variable stuff
tic
...
duration_rollingStats   = round(frameRate * 300);
subSampleFactor_runningVals = 40;
% duration_rollingStats/subSampleFactor_runningVals
...
%         runningVals = logger.decoder.rawVals([(counter_frameNum - duration_rollingStats):counter_frameNum],:);
        runningVals = logger_valsROIs([(counter_frameNum - duration_rollingStats):subSampleFactor_runningVals:counter_frameNum],:);
    end
...
%         runningVals = [nan(duration_rollingStats, 1) ; logger.decoder.rawVals(1:counter_frameNum, :)];
%         runningVals = [nan(duration_rollingStats, numCells) ; logger_valsROIs(1:counter_frameNum, :)];
        runningVals = logger_valsROIs([1:subSampleFactor_runningVals:counter_frameNum],:);
    else if counter_frameNum == 1
            runningVals = [nan(duration_rollingStats, numCells) ; vals_neurons'];
        end
    end
% toc
% tic
    vals_smooth = nanmean(logger_valsROIs(counter_frameNum-win_smooth : counter_frameNum,:),1);
% toc
%     tic
    F_baseline = prctile_2017a(runningVals, F_baseline_prctile , 1);
%     tic
    F_std = nanstd(runningVals , [] , 1);
    F_std(isnan(F_std)) = 999999999; % in rare cases a neuron will have all NaNs as lambda (pixel weighting values), so this is a catch to set it to 0.
%     F_std
%     toc
%     F_baseline = 1;
%     toc
    F_baseline(F_baseline < 0.1) = NaN;
%     F_baseline = ones(1, size(runningVals,2));
